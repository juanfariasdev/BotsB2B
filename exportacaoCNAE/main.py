...
os.mkdir(f'./exportado/{timestr}/Ativadas')
os.mkdir(f'./exportado/{timestr}/Desativadas')
...
class BotCsv:
    ...
    def ler_csv(self):
        situacao = [1, 2]
        self.AtivaDesativada = ""

        self.df = pd.read_excel("cnae.xlsx",
        ...
        for linha in range(0, numero_linhas):
            ...
            self.estado = (self.df['ESTADO'].values[linha])
            ...
            self.folder = timestr
            os.mkdir(
                f'./exportado/{timestr}/Ativadas/Distribuidor - {self.distribuidor}')
    ...
    def ler_nome_lojas(self):
        ...
        response = requests.get(
            f'http://localhost:3001/consulta/superconsulta?cnae={self.pracas}&uf={self.estado}&SituacaoEmpresa=2&porte=5')
    ...
    def criar_arquivo(self):
        ...
        writer = pd.ExcelWriter(
            f"./exportado/{timestr}/Ativadas/Distribuidor - {self.distribuidor}/{self.pracas}.xlsx", engine='openpyxl', type='3_color_scale')
